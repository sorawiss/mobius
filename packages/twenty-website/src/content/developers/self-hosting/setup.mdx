...
<ArticleTip>
If the worker log still prints the full HTML body of the reset email (for example the footer with links to Website, Github, User guide, and Developers) then the `LOGGER` driver is still active. Update the **EMAIL_DRIVER** variable from the Admin Panel or temporarily set `IS_CONFIG_VARIABLES_IN_DB_ENABLED=false` so that your container picks up the `EMAIL_DRIVER=smtp` value from the environment before trying again.
</ArticleTip>
